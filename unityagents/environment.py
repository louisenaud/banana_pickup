...
class UnityEnvironment(object):
    def __init__(
        ...
        for brain_param in aca_params.brain_parameters:
            ...
            self._brains[brain_param.brain_name] = BrainParameters(
        ...
        if self._num_external_brains == 0:
            logger.warning(
                " No External Brains found in the Unity Environment. "
    ...
    def __str__(self):
        ...
        Reset Parameters :\n\t\t{4}""".format(
                ...
                self._curriculum.get_lesson_number,
                "\n\t\t".join(
                    [
    ...
    def step(self, vector_action=None, memory=None, text_action=None) -> AllBrainInfo:
        ...
        if self._loaded and not self._global_done and self._global_done is not None:
            if isinstance(
                vector_action, (int, np.int_, float, np.float_, list, np.ndarray)
            ):
            ...
            for b in self._external_brain_names:
                ...
                if b not in vector_action:
                    ...
                    if self._brains[b].vector_action_space_type == "discrete":
                        ...
                        vector_action[b] = (
                            [0.0] * n_agent * self._brains[b].vector_action_space_size
                        )
                else:
                ...
                if not (
                    (
                        self._brains[b].vector_action_space_type == "discrete"
                        and len(vector_action[b]) == n_agent
                    )
                    or (
                        self._brains[b].vector_action_space_type == "continuous"
                        and len(vector_action[b])
        ...
        for b in output.agentInfos:
            ...
            _data[b] = BrainInfo(
                ...
                vector_observation=np.array(
                    [x.stacked_vector_observation for x in agent_info_list]
                ),
